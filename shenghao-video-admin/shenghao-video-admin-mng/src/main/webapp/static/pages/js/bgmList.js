var deleteBgm = function(bgmId) {
	...
	var flag = window.confirm("是否确认删除？？？");//弹框让用户确认是否要删除
	if (!flag) {
		return;
	}
	...
	$.ajax({
		...
		success: function(data) {
			if (data.status == 200 && data.msg == 'OK') {
				alert('删除成功~~');
				var jqGrid = $("#bgmList");  
				jqGrid.jqGrid().trigger("reloadGrid");//代表重新刷新grid
			}
		}
	})
}
...
var BgmList = function() {
    ...
    var handleBgmList = function() {
        ...
        jqGrid.jqGrid({  
            ...
            mtype: "post",  
            styleUI: 'Bootstrap',//设置jqgrid的全局样式为bootstrap样式, 不设置的样式会比较丑
            datatype: "json",  
            ...
            colModel: [  
                ...
                { name: 'path', index: 'path', width: 50, 
                	formatter: function(cellvalue, option, rowObject){//列对象也就是bgm的路径, option少用, 行对象就是每一样的对象的全部数据
...
jQuery(document).ready(function() {//对bgmlist页面进行初始化
	BgmList.init();
});